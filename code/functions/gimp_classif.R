...
Gimp_classif = R6Class("grouped_imp_classif",
  public = list(
    ...
    shapley = function(group_df, res, n.shapley.perm = 120L) {
      ...
      data.frame(group = groups, meanShapImp = rowMeans(y[, cols]), sdShapImp = apply(y[, cols], 1 , sd))
    },
    ...
    drop_group_importance = function(group_df, resampling = cv10, measures = mse) {
      ...
      for (group in c(groups, "all")) {
        ...
        task_group = makeClassifTask(data = data_group, target = getTaskTargetNames(self$task))
    ...
    group_only_importance = function(group_df, resampling = cv10, measures = mse) {
      ...
      res[["featureless"]] =  resample(learner = makeLearner("classif.featureless"), task = self$task, measures = measures, resampling = resampling)
      res
    },
