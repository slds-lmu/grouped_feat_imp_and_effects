...
#table was generated by hand with the information of following tables:

#birhtweight:
gpfi_bwt 
gopfi_bwt$GOPFI = gopfi_bwt$mse[gopfi_bwt$features == "all"] - gopfi_bwt$mse
gopfi_bwt
LOGO_bwt %>% arrange(desc(DGI))
LOGI_bwt %>% arrange(desc(goi))
shap_bwt %>% arrange(desc(meanShapImp))

#colon
gpfi_colon
gopfi_colon %>% arrange(desc(GOPFI))
LOGO_colon %>% arrange(desc(DGI))
LOGI_colon %>% arrange(desc(goi))
shap_colon %>% arrange(desc(meanShapImp))

#fuelsubset
gpfi_fuelsubset
gopfi_fuelsubset$GOPFI = gopfi_fuelsubset$mse[gopfi_fuelsubset$features == "all"] - gopfi_fuelsubset$mse
gopfi_fuelsubset %>% arrange(desc(GOPFI))
LOGO_fuelsubset %>% arrange(desc(DGI))
LOGI_fuelsubset %>% arrange(desc(goi))
shap_fuelsubset %>% arrange(desc(meanShapImp))
